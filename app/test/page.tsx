...
import { TestCategorySelector } from "@/components/test-category-selector"
...
export default function TestPage() {
  ...
  const getCategoryTitle = () => {
    if (!selectedCategory || !currentCategory) return "Oefenvragen (TEST)"
    return `Oefenvragen ${currentCategory.name} (TEST)`
  }
  ...
  return (
    <main className="w-full min-h-screen relative">
      <div className="w-full max-w-4xl mx-auto px-3 py-3 relative z-10">
        ...
        <div className="mb-4 p-3 bg-amber-100 dark:bg-amber-900/20 border-2 border-amber-500 rounded-lg">
          <p className="text-sm font-semibold text-amber-900 dark:text-amber-200 text-center">
            🧪 TEST OMGEVING - Alle categorieën zijn hier zichtbaar voor testing
          </p>
        </div>
